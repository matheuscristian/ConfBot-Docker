...
process.env.stock = (0, fs_1.readFileSync)("./.stock").toString('utf-8');
client.on("ready", (c) => {
    const date = new Date();
...
client.on("messageCreate", (msg) => {
    ...
    else if (!msg.content.startsWith(readConfig_js_1.default.prefix)) {
        return;
    }
    ...
    commandName = commandName.substring(readConfig_js_1.default.prefix.length, commandName.length).toLowerCase(); // Retira o prefixo do comando
    let notFound = true;
    readConfig_js_1.default.commands.forEach((command) => {
    ...
    notFound ? msg.reply(readConfig_js_1.default.commandNotFoundMessage) : null;
});
// client.login(hidden.token);
client.login(readConfig_js_1.default.token);
